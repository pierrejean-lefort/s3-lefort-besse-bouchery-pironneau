...
public class SuisseComplet extends SystemTournoi{
    ...
    public boolean aparriementHomogene(List<Participe> s1, List<Participe> s2) {
     ...
     */
    public boolean newRoundAux(List<Partie> parties, List<Participe> participants, int round, int numPartie){ //todo: faire de cet algo un truc qui fonctionne
        //indice de la partie
        //cas de base chelou
        ...
        if(participants.get(0).getJoueur().nbPoint(t)==0){
            ...
            if(nivPts.size()%2==0){
                ...
                //on essaie d'apparier le premier du groupe S1 avec le premier de S2 etc...
                for(int i = 0;i<S1.size();i++){
                    if (alreadyPlayedWith(S1.get(i), S2.get(i))) { //on test si l'appariement est possible, si non, on permute S2
                        S2 = permuS2.get(k);
                        i=-1;
                ...
                //si on arrive là c'est que une permu a permi de faire des appariements, on ajoute les parties et on return true
                for(int i = 0;i<S1.size();i++){
                    //parties.add(Partie.createPartie(t,)); //todo: déterminer la couleur des joueurs à ajouter dans les parties
                }
            }
        }
        ...
        //cas récursif todo: le cas récursif
            ...
            int j=0; //indice joueur
            Participe p1 = participants.get(j);
            ...
            //cas
            if(nivPts.size()<2){ //if top player alone with its points
                nivPts = getSameScore(participants.get(1), participants);
                nivPts.add(p1);
            }
        }
        //Partie partie = Partie.createPartie(t, );
        return false;
    }
